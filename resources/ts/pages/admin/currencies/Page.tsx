...
const Content = ({ title }: { title: string }) => {
  ...
  const {
    ...
    meta: {
      ...
      json: {
        ...
        length: pageSize,
        page: current,
        columns: ["currency_code", "currency_name"].map((name) => ({
          data: name,
          name,
          orderable: true,
          searchable: true,
  ...
  const columns: TableColumnsType<any> = [
    {  
      ...
      // render: (txt: any, row: any) => currentUser?.is_sysadmin ? <Link to={"/adverts/" + row.id} tabIndex={loadingProcess ? -1 : undefined}>{txt}</Link> : txt,
    },
    {  
      title: 'Name',
    ...
    },
    {
      ...
      fixed: 'right',
      width: 75,
      render: (txt: any, { id, currency_code, currency_name, is_sysadmin }: any) => {
        return currentUser?.is_sysadmin && (
          <>
            <Button
              ...
              onClick={() => toggleFormModal({ id, currency_code, currency_name })} // push('/adverts/' + row.id)
            />
            {' '}
            {!is_sysadmin && (
              <Button
    ...
    <Header
      ...
      content={
        <>
          ...
          <Button
            type="primary"
            onClick={() => toggleFormModal({ currency_name: '' })} // show('')
          >
            Create
          </Button>
        </>
...
export default function Page() {
  ...
  const { data: accessControl } = useCan({
    ...
    params: { resource: { entity_name: "currency" } }
  });
